...
interface FindMany<T> {
    findMany(): Promise<T[]>; // Renvoie une promesse contenant un tableau de T
  }
...
interface FindOne<T> {
    findOne(id: number): Promise<T | null>; // Trouve un élément par son id
  }
  
  interface Create<T> {
    create(item: T): Promise<T>; // Créer une ressource et la retourne
  }
  
  interface Update<T> {
    update(id: number, item: T): Promise<T | null>; // Maj ressource
  }
  
  interface Delete {
    delete(id: number): Promise<void>; // Supprime une ressource & retourne rien
  }
...
class SQLStudentRepository implements StudentRepository {
    async findMany(): Promise<Student[]> {
      return []; // Retourne un tab vide
    }
  
    async findOne(id: number): Promise<Student | null> {
      return null; // Retourne null car pas encore connecté à une vraie BDD
    }
  
    async create(student: Student): Promise<Student> {
      return student; // Retourne l'objet créé
    }
  
    async update(id: number, student: Student): Promise<Student | null> {
      return null; 
    }
  
    async delete(id: number): Promise<void> {
      // Retourne rien car delete doit pas renvoyer de val
    }
  }
  ...
  class SQLCourseRepository implements CourseRepository {
    ...
    async create(course: Course): Promise<Course> {
      return course; // Retourne l'objet créé
    }
